...
# Load in data
mat = np.load("full_rep.npy")

# Project data onto top 4 PCs
pca = PCA(n_components=4)
pca.fit(mat)
pca_mat = pca.transform(mat)

# Create 2D UMAP representation
umap_rep = get_umap(pca_mat)

# Perform random subsampling
np.random.seed(42)
smol_inds = sample(inds, smol_size)
pca_smol = pca_mat[smol_inds, :]
umap_smol = umap_rep[smol_inds, :]

# Compute Z2 homology in BATS
data = bats.DataSet(bats.Matrix(pca_smol))
...
#	dgms = ripser(pca_mat, maxdim=1)["dgms"]
	dgms = ripser(pca_smol, maxdim=1)["dgms"]
	H1 = dgms[1]
	birth_lifetime(H1, ax2, y_max=120)
	...
	if k == 0:
		...
		suffix = "th"
	fig.suptitle("Top Four PCs,\n"+str(k+1)+suffix+" Longest Living H1 Feature")
	fig.savefig("four_pcs_"+str(k)+".png")
	plt.close(fig)
