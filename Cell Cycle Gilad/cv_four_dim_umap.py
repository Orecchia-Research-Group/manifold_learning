...
from cell_cycle_utils import birth_lifetime
...
for j, pair in tqdm(enumerate(top_h1_four_dim)):
	...
	ax.scatter(umap_smol[:, 0], umap_smol[:, 1], c="k", s=0.1)
	edge_x = []
	edge_y = []
...
#		ax.plot(xs, ys, c="r", lw=1.0)
	...
	if j == 0:
		...
		suffix = "th"
	fig.suptitle("Cell Cycle Genes,\n"+str(j+1)+suffix+" Largest H1 Feature")

	fig.savefig("which_cycle_"+str(j)+".png")
	fig.savefig("cell_cycle_genes_"+str(j)+".png")
	plt.close(fig)
