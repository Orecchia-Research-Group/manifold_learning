...
def plot_loadings(ax, azalia, num_loadings=5):
	...
	for ind in inds_for_loading:
#		ax.arrow(0, 0, azalia[ind, 0], azalia[ind, 1], color="r", lw=0.02)
		ax.plot(azalia[ind, 0], azalia[ind, 1], "o", ms=5)
	ax.axvline(0, color="k")
	ax.axhline(0, color="k")
	ax.legend(genes_for_loading)

for macro_ind, azalia in zip(indices, azalias):
	fig = plt.figure()
	ax = fig.add_subplot(111)
	plot_loadings(ax, azalia)
	fig.savefig("loadings/"+str(macro_ind)+".pdf")
