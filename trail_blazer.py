...
from manifold_utils.mSVD import Sparse_eigen_calc_from_dist_mat
...
for j, ind in enumerate(trail_indices):
	start = time()
	radii, _, eigval_list, eigvec_list = Sparse_eigen_calc_from_dist_mat(sparse_var, dist_mat, ind, np.max(dist_mat), k=10, radint=0.1)
